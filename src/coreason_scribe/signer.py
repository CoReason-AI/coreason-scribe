...
class SigningRoom:
    ...
    def sign(
        self, artifact: DraftArtifact, user_context: UserContext, signer_role: str, credential: str
    ) -> DraftArtifact:
        """
        ...
        # Authenticate
        token = self.identity_provider.authenticate(user_context.sub, credential)
        if not token:
        ...
        signature = SignatureBlock(
            ...
            signer_id=user_context.sub,
            signer_role=signer_role,
            authority=user_context.permissions,
        ...
        logger.info(f"Artifact version {artifact.version} signed by {user_context.sub}.")

        return artifact

    def sign_draft(self, artifact: DraftArtifact, user_context: UserContext) -> DraftArtifact:
        """
        Signs a DRAFT document for generation attribution.
        """
        # Calculate Hash
        content_to_hash = f"{artifact.version}:{artifact.timestamp.isoformat()}"
        for section in artifact.sections:
        ...
        signature = SignatureBlock(
            document_hash=document_hash,
            signer_id=user_context.sub,
            signer_role="Generator",
            authority=user_context.permissions,
            timestamp=datetime.now(timezone.utc),
            meaning="Generated by Scribe Engine",
            signature_token=f"gen-token-{uuid.uuid4()}",
        )

        artifact.signature = signature
        logger.info(f"Artifact version {artifact.version} attributed to {user_context.sub}.")
