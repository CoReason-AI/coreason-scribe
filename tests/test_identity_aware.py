...
from coreason_scribe.main import ScribeError, main, run_inspect, run_sign, run_verify
from coreason_scribe.signer import ScribeSigner


@pytest.fixture
def mock_context() -> UserContext:
    return UserContext(user_id=SecretStr("test-user"), roles=["tester"], metadata={})
...
def test_signer_sign_pdf(mock_context: UserContext, pdf_path: Path) -> None:
    signer = ScribeSigner()
    # It should succeed
    signer.sign_pdf(pdf_path, mock_context)


def test_signer_verify_signature(mock_context: UserContext, pdf_path: Path) -> None:
    signer = ScribeSigner()
    assert signer.verify_signature(pdf_path, mock_context) is True


def test_inspector_inspect_pdf(mock_context: UserContext, pdf_path: Path) -> None:
    inspector = ScribeInspector()
    res = inspector.inspect_pdf(pdf_path, mock_context)
    assert isinstance(res, dict)


def test_missing_context_raises(pdf_path: Path) -> None:
    signer = ScribeSigner()
    with pytest.raises(ValueError):
        signer.sign_pdf(pdf_path, None)


def test_missing_file_raises(mock_context: UserContext, tmp_path: Path) -> None:
    signer = ScribeSigner()
...
# Coverage tests
def test_inspector_missing_context(pdf_path: Path) -> None:
    inspector = ScribeInspector()
    with pytest.raises(ValueError):
        inspector.inspect_pdf(pdf_path, None)


def test_inspector_missing_file(mock_context: UserContext, tmp_path: Path) -> None:
    inspector = ScribeInspector()
    with pytest.raises(FileNotFoundError):
        inspector.inspect_pdf(tmp_path / "nonexistent.pdf", mock_context)


def test_signer_verify_missing_context(pdf_path: Path) -> None:
    signer = ScribeSigner()
    with pytest.raises(ValueError):
        signer.verify_signature(pdf_path, None)


def test_signer_verify_missing_file(mock_context: UserContext, tmp_path: Path) -> None:
    signer = ScribeSigner()
    with pytest.raises(FileNotFoundError):
...
def test_cli_sign(mock_uc_cls: MagicMock, mock_signer_cls: MagicMock, pdf_path: Path) -> None:
    ...
    # Since MockSecretStr doesn't implement __eq__, we check the value manually
...
@patch("coreason_scribe.main.ScribeSigner")
def test_cli_verify(mock_signer_cls: MagicMock, pdf_path: Path) -> None:
    mock_signer = mock_signer_cls.return_value
    run_verify(pdf_path)
...
def test_cli_inspect(mock_inspector_cls: MagicMock, pdf_path: Path) -> None:
...
# CLI Exception tests
@patch("coreason_scribe.main.ScribeSigner")
def test_cli_sign_error(mock_signer_cls: MagicMock, pdf_path: Path) -> None:
    mock_signer = mock_signer_cls.return_value
...
@patch("coreason_scribe.main.ScribeSigner")
def test_cli_verify_error(mock_signer_cls: MagicMock, pdf_path: Path) -> None:
    mock_signer = mock_signer_cls.return_value
...
@patch("coreason_scribe.main.ScribeInspector")
def test_cli_inspect_error(mock_inspector_cls: MagicMock, pdf_path: Path) -> None:
    mock_inspector = mock_inspector_cls.return_value
...
def test_main_sign(mock_run: MagicMock, pdf_path: Path) -> None:
...
def test_main_verify(mock_run: MagicMock, pdf_path: Path) -> None:
...
def test_main_inspect(mock_run: MagicMock, pdf_path: Path) -> None:
