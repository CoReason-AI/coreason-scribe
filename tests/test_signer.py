...
def test_tampering_after_approval(signing_room: SigningRoom, draft_artifact: DraftArtifact) -> None:
    """
    ...
    # 3. Sign
    signing_room.sign(draft_artifact, "signer", "role", "correct-password")
    ...
    # Verify it implies the original content would have produced a different hash
    content_original = f"{draft_artifact.version}:{draft_artifact.timestamp.isoformat()}"
    content_original += (
        f":{draft_artifact.sections[0].id}:{original_content}:{draft_artifact.sections[0].linked_code_hash}"
    )
    original_hash = hashlib.sha256(content_original.encode("utf-8")).hexdigest()

    assert draft_artifact.signature.document_hash != original_hash


def test_empty_artifact_signing(signing_room: SigningRoom) -> None:
    """
    ...
    # Approved -> Signed
    signing_room.sign(empty_artifact, "signer", "role", "correct-password")
...
def test_unicode_content_signing(signing_room: SigningRoom) -> None:
    """
    ...
    signing_room.sign(unicode_artifact, "signer", "role", "correct-password")
...
def test_strict_state_transitions_idempotency(signing_room: SigningRoom, draft_artifact: DraftArtifact) -> None:
    """
...
def test_signature_integrity_verification(signing_room: SigningRoom, draft_artifact: DraftArtifact) -> None:
    """
